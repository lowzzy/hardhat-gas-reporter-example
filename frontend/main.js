import todoContract from '../artifacts/contracts/Gacha.sol' assert { type: 'json' };
...
function createTodo(msg) {
  createABI(msg).then(() => displayTodo().then($('#createTodoInput').val('')));
}
...
function _updateDisplay(todoList) {
  ...
  for (const e of todoList) {
    ...
    todoHTMLItems =
      ...
      '<li id="' +
      e.id +
      '" class="list-group-item border-0 d-flex align-items-center ps-0"><input class="deleteTodoButton btn btn-danger" type="button" value="削除"/>　　<input class="form-check-input me-3" type="checkbox" value="" aria-label="..." ' +
      checkFlag +
      ' />' +
      e.contents +
      '</li>';
